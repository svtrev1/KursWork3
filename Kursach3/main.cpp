...
public:
    ...
    int hashFunction1(K key, int newSize) // Общая хеш-функция
    {
        string temp;
        if (typeid(key).name()[0] == 'S')
            temp = key;
...
//    int tableSize; // Размер хеш-таблицы
//    static const int initialSize = 7; // Начальный размер таблицы
//    static const int resizeFactor = 2; // Коэффициент увеличения размера mтаблицы
//
//    vector<list<pair<K, V>>>* table; // Массив для хранения пар ключ-значение
//    int numElements; // Количество элементов в таблице
//public:
//    HashTable()
//    {
//        tableSize = initialSize;
//        table = new vector<list<pair<K,V>>>[tableSize]();
//        numElements = 0;
//    }
//    int hashFunction(K key) // Общая хеш-функция
//    {
...
//        table[index].push_back(make_pair(key, value));
//        numElements++;
//        if (((double)(numElements) / tableSize) > 70)
//            resizeTable();
//    }
//    
//    void printTable() 
...
//    void resizeTable() 
//    {
//        tableSize *= resizeFactor; // Увеличиваем размер таблицы
...
//            {
//                int index = hashFunction(pair.first) % size; // Перехешируем все существующие элементы в новую таблицу
